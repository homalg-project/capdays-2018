...
\begin{frame}[fragile]
 ...
 \pause
  ...
  \end{itemize}
 \begin{block}{Example: $\Q\mathrm{\text{-}vec}$}\pause
 \begin{center}
      \begin{tikzpicture}[transform shape,mylabel/.style={thick, draw=black, align=center,minimum width=0.5cm, minimum height=0.5cm,fill=white}]
      \coordinate (r) at (3,0);
      \coordinate (d) at (0,-1);
...
\begin{frame}[fragile]
 \frametitle{Implementation of the kernel: $\Q\mathrm{\text{-}vec}$}
 \begin{block}{}
  $\Obj := \Z_{\geq 0}$, $\Hom \left( m, n \right) := \Q^{m \times n}$
 \end{block}
 \pause
 \begin{block}{}
  \begin{center}
...
%  \tikz[remember picture,overlay] \node[opacity=0.3,inner sep=0pt] at (current block.center){\includegraphics[width=\blockwidth,height=\blockheight]{Logo_CAP.jpg}};
  \begin{center}
   \begin{tikzpicture}[node distance=0.7cm,scale=0.6, every node/.style={scale=0.6}]
    ...
    \node (captotal) at ($(cap)+(1,0)$) [opacity=.40] {\includegraphics[width=1.4\textwidth]{Logo_CAP.jpg}};
    \end{scope}
    }
    
    \visible<3->{
    \node (ambient) [boxpackage, left = of attribute, fill=\myblue, xshift = -3em] {CategoriesWithAmbientObjects};
    \node (m2) [boxpackage, below = of ambient, fill=\myblue] {M2};
    ...
    \visible<6->{
    \node (motives) [boxpackage, below = of m2, fill=\myblue, xshift = -2em] {MotivesForBiArrangements};
    
    }
    \visible<6>{
        \draw[->,thick] (motives) |- (m2);
    }
    \visible<8->{
...
%     \visible<10->{
%     \node (gradedmod) [boxpackage, right = of modulepres, fill=green!20] {GradedModulePresentations};
%     \node (toricsheaves) [boxpackage, below = of gradedmod, fill=green!20] {ToricSheaves};
%     \draw[->,thick] (gradedmod) -- (modulepres);
%     \draw[->,thick] (toricsheaves) -- (gradedmod);
%     }
